...
const FondoCorazonComponent: React.FC<HeartsBackgroundProps> = ({ count = 60, tipo }) => {
  const { luz } = useMiContexto();

  // Asignamos las imágenes aleatorias una sola vez por cada punto en useMemo
  ...
  const claseFondo = luz ? tipo : "star";

  const renderSimbolo = (heart: HeartConfig) => {
    if (!luz) return "⛧";
  ...
  return (
    <div className={`${claseFondo} fondo`}>
      {hearts.map((heart) => (
        <span
          key={heart.id}
          className={`${claseFondo} simbolo`}
          style={{
            top: heart.top,
            left: heart.left,
...
// Evita re-renders innecesarios cuando cambian componentes hermanos
export const FondoCorazon = React.memo(FondoCorazonComponent);
